...
func (s *Store) GetSetting(key string) (string, error) {
	var value string
	err := s.db.QueryRow(`SELECT value FROM qianwen_settings WHERE key=?`, key).Scan(&value)
	if err != nil {
		return "", err
	}
	return value, nil
...
func (s *Store) SetSetting(key, value string) error {
	_, err := s.db.Exec(`INSERT INTO qianwen_settings (key, value, updated_at)
		VALUES (?, ?, ?)
...
func accountSupportsCapability(a AccountRecord, capability string) bool {
	...
	if capability != "chat" && a.Type != "guest" && !accountHasQianwenLoginMaterial(a) {
		return false
	}
	if a.CapabilitiesJSON == "" {
